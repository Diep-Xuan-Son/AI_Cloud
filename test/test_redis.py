...
# key_arr = redisClient.hkeys("FaceFeature")
# value_arr = np.array(redisClient.hvals("FaceFeature"))
# features = np.frombuffer(value_arr, dtype=np.float16).reshape(len(value_arr), 1, 512)
# print(b'-;'.join(key_arr).decode('utf-8').split("-;"))
# print(features.shape)

# face_infor = redisClient.hget("FaceInfor", key_arr[0])
# print(face_infor.decode("utf-8").split("@@@"))

# infor_arr = np.array(redisClient.hvals("FaceInfor"))
# print(infor_arr)
# print(np.array(b'@@@'.join(infor_arr).decode('utf-8').split("@@@")).reshape(-1,3))

# print(redisClient.hkeys("FaceFeature1"))

print(len(redisClient.hkeys("FaceInfor")))